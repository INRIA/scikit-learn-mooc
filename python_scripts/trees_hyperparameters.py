...
def fit_and_plot_classification(model, data, feature_names, target_names):
    ...
    DecisionBoundaryDisplay.from_estimator(
        model, data[feature_names], response_method="predict", cmap="RdBu", alpha=0.5
    )
    sns.scatterplot(
        data=data,
        x=feature_names[0],
...
def fit_and_plot_regression(model, data, feature_names, target_names):
    ...
    sns.scatterplot(x=data.iloc[:, 0], y=data[target_names], color="black", alpha=0.5)
...
# %%
fit_and_plot_classification(tree_clf, data_clf, data_clf_columns, target_clf_column)
_ = plt.title(f"Shallow classification tree with max-depth of {max_depth}")

# %%
fit_and_plot_regression(tree_reg, data_reg, data_reg_columns, target_reg_column)
...
# %%
fit_and_plot_classification(tree_clf, data_clf, data_clf_columns, target_clf_column)
_ = plt.title(f"Optimal depth found via CV: " f"{tree_clf.best_params_['max_depth']}")

# %%
fit_and_plot_regression(tree_reg, data_reg, data_reg_columns, target_reg_column)
_ = plt.title(f"Optimal depth found via CV: " f"{tree_reg.best_params_['max_depth']}")
